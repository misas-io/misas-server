...
export function run(parms){
  ...
  co(function* (){
    ...
    var removed = yield events.remove({date: {$lt: currentTime }});
    var removed = yield events.remove({date: {$lt: currentTime, $gte: from,  $lte: until}});
    log.info(`removed ${removed.result.n} events with dates less than ${currentTime}`);
    grps.find({}).forEach(
      (grp) => {
        count += 1;
