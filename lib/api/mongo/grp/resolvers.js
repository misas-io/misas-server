...
export const GrpQueryResolvers = {
  ...
  searchGrps: (
    ...
    let fromTS;
    if (!parms.time) {
      fromTS = moment().tz('America/Denver');
    } else {
      fromTS = moment(parms.time);
    }
    let localTS = moment();
    let diffTZ = fromTS.utcOffset() - localTS.utcOffset();
    log.verbose(`from: ${fromTS.format()}`);
    log.verbose(`to: ${localTS.format()}`);
    log.verbose(`offset: ${diffTZ}`)
    let currentTS = moment().utc().add(diffTZ, 'm').format();
    log.verbose(currentTS);
    currentTS = moment(currentTS).toDate();
    log.verbose(`search_ts: ${currentTS}`);
    context.localtime = currentTS;
    return co(function* (){
...
export const GrpResolvers = {
  ...
  Grp: {
    ...
    nextEvents(grp, { next }, context) {
      let nextV = !isInteger(next) ? 10 : 0 <= next && next <= 100 ? next : 10;
      return getNextGrpDatesFromUntil(grp, next, context.localtime);
    }
  },
  Recurrence: {
  }
